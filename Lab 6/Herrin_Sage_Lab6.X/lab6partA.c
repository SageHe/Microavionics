...
void Initial() {
    ...
    ONcount = 59286; // for a 0.1 second on
    OFFcount = 9286; // for a 0.9 second off 
    LATDbits.LATD4 = 0b0;
    ...
    PIE1bits.TX1IE = 0b1; // set high, enable EUSART Transmit interrupt 
...
void __interrupt(low_priority) LoPriISR(void) 
{
    ...
    while(1) {
        ...
        else if(PIR1bits.TX1IF ) { // ready to transmit
            // call the transmit function
            transmit();
        }
        else if(PIR1bits.RC1IF ) { // message is in the register
