...
unsigned int ADresult = 0;  // result of AD Conversion
unsigned int LM35result = 0;
float Temp = 0;
float Temp1 = 0;
unsigned int P2result = 0;
...
char temperature[10]; // temporary holder for temp
char voltage[10]; // temporary holder for pot. voltage
...
void potC(void);                // conversion function for P2
...
void main() {
     Initial();                 // Initialize everything
      while(1) {
        ...
        //ADCON0bits.CHS = P2channel; // 00011 for A03, correct channel chosen for LM35
        // add delay here for switching channels, up to 10 us, can use __delay
        ...
        //potC(); // converts the converted binary into voltage value
        //volts(); 
...
{
    // go on AD converter 
    for(samplecount = 0; samplecount<7; samplecount++) {
        // initialize done
        __delay_us(10); //wait 10 micro seconds after changing channels
        ...
        LATDbits.LATD5 = ~LATDbits.LATD5;// DEBUGGING ONLY
        // above line was found from datasheet pg 393. 
...
void tempC()
{
    ...
    // [V]*1/[V/C_lm35] = output
    // temperature = LM35result * 3.3/4096[V/bin] * 1/((0.01)[V/C]) = 0.0806
    Temp = Cscalefactor * LM35result;
    // convert type double Temp to 3 strings:
    sprintf(temperature, " T=%.1fC ", Temp);
    // conversion found at following web address: tested for this purpose, works
    // https://stackoverflow.com/questions/8257714/how-to-convert-an-int-to-string-in-c
    temperature[0] = 0x80;
    // now display T = [temp] C:
    // Change "temperature" string bits 4, 5, and 7. 
    
    // will be printed back in main loop
}
...
void potC()
{
    //adc output = bin#
    //want: voltage: -> bin# * voltage/bin# -> bin# * (3.3/2^12)
    Temp1 = P2result * vscalefactor;
    sprintf(voltage," PT=%.2fV",Temp1);
    voltage[0] = 0xC0;
