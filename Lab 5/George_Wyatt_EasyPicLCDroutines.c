...
void DisplayV( const char * LCDStr ) {
    ...
    while(LCDStr[count]!=0x00) {// Send Each Byte one nibble at a time until we see the null character
        ...
        LCD_E_LAT = 1;
   	    nibble = 0xF0 & LCDStr[count];        // Mask to get upper nibble of LCD string
        nibble >>= 4;            // Shift right by 4 to make lower nibble for data ports
  		LCD_DATA_LAT |= nibble;    // Mask the upper nibble to ONLY last four of PORTB(RB3:0)
        LCD_E_LAT = 0;
